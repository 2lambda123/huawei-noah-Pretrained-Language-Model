...
from schedulers import LinearWarmUpScheduler, ConstantWarmUpScheduler
...
def remove_ignore_index(target, preds, ignore_index):
...
class BertPretrainingValidation(torch.nn.Module):
    def __init__(self, vocab_size):
        super(BertPretrainingValidation, self).__init__()
        self.vocab_size = vocab_size
    def forward(self, prediction_scores, masked_lm_labels):
        acc = (prediction_scores.view(-1, self.vocab_size).argmax(1) == masked_lm_labels.view(-1)).sum().item()
        acc = acc / masked_lm_labels.view(-1).shape[0]
        return acc
...
def parse_arguments():
    ...
    parser.add_argument("--dev_dir",
                        ...
                        type=str,
                        required=True,
                        help="The dev data dir. Should contain .hdf5 files  for the task.")
    ...
    parser.add_argument('--json-summary', type=str, default="/cache/results/dllogger.json",
    ...
    parser.add_argument("--optimizer",
                        default='came',
                        help="Optimizer for pretraining.")
...
def prepare_model_and_optimizer(args, device):
    ...
    if args.optimizer == 'adafactor':
        ...
        lr_scheduler = LinearWarmUpScheduler(optimizer, warmup=args.warmup_proportion, total_steps=args.max_steps)
        ...
        optimizer = SM3(optimizer_grouped_parameters, 
                    lr=args.learning_rate,
                    momentum=0.95)
        lr_scheduler = ConstantWarmUpScheduler(optimizer, 
                                    warmup=args.warmup_proportion, 
                                    total_steps=args.max_steps)
    ...
    validation = BertPretrainingValidation(config.vocab_size)

    return model, optimizer, lr_scheduler, checkpoint, global_step, criterion, names, validation
...
def main():
    ...
    # Prepare optimizer
    model, optimizer, lr_scheduler, checkpoint, global_step, criterion, names, validation = prepare_model_and_optimizer(args, device)
            ...
            for f_id in range(f_start_id + 1 , len(files)):
                ...
                for step, batch in enumerate(train_iter):
                    ...
                    if global_step >= args.steps_this_run or timeout_sent:
                        ...
                        if is_main_process():
                            if len(optimizer.state) > 0:
                                id_list = optimizer.state_dict()['param_groups'][0]['params']
                                #print(id_list)
                                name_list = dict()
                                assert len(id_list) == len(names)
                                for i in range(len(names)):
                                    name_list[names[i]] = id_list[i]
                    ...
                    if global_step >= args.steps_this_run or training_steps % (
                      100 * args.num_steps_per_checkpoint * args.gradient_accumulation_steps) == 0 or timeout_sent:
                        if is_main_process() and not args.skip_checkpoint:
                            ...
                            if args.do_train:
                                torch.save({'model': model_to_save.state_dict(),
                                            ...
                                            'files': [f_id] + files,
                                            'epoch': epoch,
                                            'data_loader': None if global_step >= args.max_steps else train_dataloader}, output_save_file, pickle_protocol=4)
                        ...
                        if args.do_eval:
                                dev_files = [os.path.join(args.dev_dir, f) for f in os.listdir(args.dev_dir) if os.path.isfile(os.path.join(args.dev_dir, f))]
                                
                                num_files = len(dev_files)
                                f_id = 0
                                if torch.distributed.is_initialized() and get_world_size() > num_files:
                                    remainder = get_world_size() % num_files
                                    data_dev_file = dev_files[(f_id*get_world_size()+get_rank() + remainder*f_id)%num_files]
                                else:
                                    data_dev_file = dev_files[(f_id*get_world_size()+get_rank())%num_files]
                                model.eval()

                                dev_data = pretraining_dataset(data_dev_file, args.max_predictions_per_seq)
                                dev_dataloader = DataLoader(dev_data, 
                                                            batch_size=args.dev_batch_size * args.n_gpu,
                                                            num_workers=4, worker_init_fn=worker_init,
                                                            pin_memory=True)

                                dev_iter = tqdm(dev_dataloader, desc="Iteration", disable=args.disable_progress_bar) if is_main_process() else dev_dataloader
                                ...
                                with torch.no_grad():
                                    for step, batch in enumerate(dev_iter):
                                        ...
                                        input_ids, segment_ids, input_mask, masked_lm_labels, next_sentence_labels = batch
                                        prediction_scores, seq_relationship_score = model(input_ids=input_ids, token_type_ids=segment_ids, attention_mask=input_mask)
                                        vocab_size = prediction_scores.shape[2]
                                        
                                        preds = prediction_scores.view(-1, vocab_size)
                                        target = masked_lm_labels.view(-1)
                                        target, preds = remove_ignore_index(target, preds, ignore_index=-1)
                                        acc = accuracy(preds, target)
                                    acc = accuracy.compute()
                                    
                                    
                                    del dev_dataloader
                                    dllogger.log(step=(epoch, global_step, ), data={"val_acc": acc.item(), "rank": get_rank()})
                                    accuracy.reset()

...
class XnliProcessor(DataProcessor):
    ...
    def get_train_examples(self, data_dir):
        ...
        for (i, line) in enumerate(lines):
            ...
            guid = "train-%d" % (i)
            if len(line) != 3:
                print('wrong train line: %s: ' + str(line))
                continue
            text_a = line[0]
            text_b = line[1]
            label = line[2]
...
def main():
    ...
    if args.bert_model:
        ...
        tokenization.BertTokenizer(vocab_file=os.path.join(args.trained_model_dir, 'vocab.txt'), do_lower_case=True)
    ...
    if args.do_train:
        ...
        for _ in trange(int(args.num_train_epochs), desc="Epoch"):
            ...
            output_model_file = os.path.join(args.output_dir, WEIGHTS_NAME + '.ep' + str(num_epoch))
    ...
    if args.do_eval and (args.local_rank == -1 or torch.distributed.get_rank() == 0):
        ...
        eval_result_file = os.path.join(args.output_dir, "eval_all_results.txt")
        with open(output_eval_file, "a") as writer:
